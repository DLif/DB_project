...
public class FactsFile_parser implements File_parser{
	...
	static int isLeaderOfNum = 0;
	static int locatedInNum = 0;
	static int hasCapitalNum = 0;
	static int hasCurrencyNum = 0;
	static int hasOfficialLanguageNum = 0;
	static int ownsNum = 0;
	static int diedInNum = 0;
	static int bornInNum = 0;
	...
	public void init() {
		...
		Maps_entitys.leadersMap = new HashMap<String,Leader_entity>(3000);
		Maps_entitys.constructionsMap = new HashMap<String,Construction_entity>();
	}
	...
	public void filter(String line){
		...
		if (relation.contains(happenedIn_relation)){
			...
			Location_entity location = Maps_entitys.locationsMap.get(right_entity);
				
			if (location == null) {
				return;
			}
			else {
				Leader_entity leader = Maps_entitys.leadersMap.get(left_entity);
				if (leader == null){
					return;
				}
				else {
	...
	private void owns_mapSetter(String line) {
		...
		//now get the proper objects from the maps and set relation reference
		Location_entity location = Maps_entitys.locationsMap.get(left_entity);
		...
		if (location == null) {
			...
			Maps_entitys.constructionsMap.put(right_entity,construction);
		}
	}
	...
	private void hasOfficialLanguage_mapSetter(String line) {
		...
		if (location == null || !(location instanceof Country_entity)) {
			...
			Language_entity officialLang = Maps_entitys.langugagesMap.get(right_entity);
	...
	private void hasCurrencyRelation_mapSetter(String line) {
		...
		if (location == null || !(location instanceof Country_entity)) {
			...
			Currency_entity currency = Maps_entitys.currenciesMap.get(right_entity);
			if (currency == null){
				return;
			}
			else {
	...
	private void isLeaderOfRelation_mapSetter(String line) {
		...
		if (location == null) {
			...
			isLeaderOfNum++;
			if (Maps_entitys.leadersMap.get(left_entity) != null){
				Maps_entitys.leadersMap.get(left_entity).addLeaderOf(location);
			}
				...
				Maps_entitys.leadersMap.put(left_entity,leaderOfLocation);
			}
		}
	}
	...
	private void locatedInRelation_mapSetter(String line) {
		...
		//now get the proper objects from the maps and set relation reference
		Location_entity inlocation = Maps_entitys.locationsMap.get(left_entity);
		...
		if (inlocation == null) {
			...
			if (inlocation instanceof City_entity){ //check if city
				Location_entity outerlocation = Maps_entitys.locationsMap.get(right_entity);
			...
			//second case - country in continent
				...
				Continent_entity continentForCity = Maps_entitys.continentsMap.get(right_entity);
	...
	private void hasCapitalRelation_mapSetter(String line) {
		...
		//now get the proper objects fro the maps and set relation reference
		Location_entity capitalCity = Maps_entitys.locationsMap.get(right_entity);
		if (capitalCity == null || !(capitalCity instanceof City_entity)){
			return;
		}
		else{
			Location_entity  countryLoc = Maps_entitys.locationsMap.get(left_entity);
